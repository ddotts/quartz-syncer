...
describe("PublishStatusManager", () => {
	it.each(["characters/Ada.md", "Ada.md"])(
		"uses %s for remote status comparison and deleted-note detection",
		async (publishPath) => {
			const compiledFile = {
				getCompiledFile: jest
					.fn()
					.mockReturnValue(["compiled", { blobs: [] }]),
				getPublishPath: jest.fn().mockReturnValue(publishPath),
			};
